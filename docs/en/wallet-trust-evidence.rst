...
Below a non-normative example of the JSON structure.
...
.. note:: To emulate key attestation, the ability to obtain a so-called assertion comes to the rescue. An assertion is generated for creating the credentials key and it is signed by using the same key backing a previously obtained attestation. By that logic, computing an assertion over the public key of a freshly generated public/private key pair proves that an authentic, uncompromised app on a non-jailbroken device was used to generate this key pair as intended by the developer. Here, we put the trust on the developer.

.. note:: An alternative to this solution is to constructs a structured (JSON) client data object (``key_hash``), inspired by WebAuthn and passes tha hash of this data to DIS. As the attestation proof in the case of iOS contains AppAttest attestation statement together with the assertion. Here the source of the trust is on the DIS and not the developer.
...
The Wallet Instance MUST do an HTTP request to the Wallet Provider's `wallet trust evidence endpoint`_,
using the method `POST <https://datatracker.ietf.org/doc/html/rfc6749#section-3.2>`__.

The **wallet trust evidence** endpoint (act as an token endpoint which is defined in `RFC 7523 section 4`_) requires the following parameters
...
It MAY contain the following OPTIONAL claims:
...
.. list-table::
    ...
    * - **Claim**
      ...
      - **Reference**
    * - **certification**
      - A String that contains a URL that links to the certification of the key storage component.
      - `OpenID4VCI`_.  
    * - **status**
      - JSON Object representing the supported revocation check mechanisms, such as the one defined in OAuth Status List.
      - `OpenID4VCI`_.  
