...
- Il Fornitore di Wallet DEVE garantire che un Wallet Unit non revocato presenti in ogni momento una Wallet App Attestation temporalmente valida e non revocata a un PID Provider o a un Attestation Provider durante il processo di emissione di un PID o di un'attestazione. Nota: Questo requisito si applica sia alle attestazioni legate al dispositivo (device-bound) sia a quelle non legate al dispositivo (non-device-bound), come definito in EIDAS-ARF.  `EIDAS-ARF`_.
- Un Wallet Unit DEVE presentare una Wallet App Attestation solo a un PID Provider o a un Attestation Provider, come parte del processo di emissione di un PID o di un'attestazione, e non a una Relying Party o a qualsiasi altra entità.
...
- Durante l'emissione di un PID, il Wallet Unit DEVE fornire al PID Provider una WUA valida che descriva il WSCA/WSCD che ha generato la nuova chiave privata del PID. Nota: Una chiave privata del PID è sempre generata e gestita dal WSCA/WSCD, che per definizione soddisfa i requisiti del Livello di Garanzia Alto (Level of Assurance High).
- Durante l'emissione di un attestazione legata al dispositivo (device-bound), un Wallet Unit DEVE recuperare i requisiti dell'Attestation Provider relativi alla memorizzazione delle chiavi da parte del WSCA/WSCD dai metadati dell'Issuer (come specificato in `OpenID4VCI`_). Il Wallet Unit DEVE determinare quale dei propri WSCA/WSCD, se presente, soddisfa tali requisiti. Se un WSCA/WSCD o keystore conforme è disponibile per il Wallet Unit, quest'ultimo DEVE fornire all'Attestation Provider una WUA valida che descriva il WSCA/WSCD o il keystore selezionato. Nota: Una WUA descrive le proprietà del WSCA/WSCD o di un keystore e contiene una o più chiavi pubbliche corrispondenti alle chiavi private generate e memorizzate in tale WSCA/WSCD o keystore.
...
- Una Wallet Unit DEVE presentare una Wallet Unit Attestation solo come parte del processo di emissione di un PID o di un'attestazione legata alla chiave (key-bound attestation).
...
- La Wallet Unit Attestation DEVE essere firmata dal Wallet Provider che ha autorità sulla Wallet Solution ed è il proprietario della stessa, come specificato dall'Autorità di Registrazione competente. I Wallet Provider DEVONO garantire che i certificati utilizzati per firmare le WUA e le WAA siano conformi a tutti i requisiti applicabili definiti in `ETSI TS 119 412-6`_, in particolare alla Clausola 5.
- Un Attestation Provider che emette attestazioni non legate al dispositivo (non-device-bound) DEVE indicare nei propri metadati del Credential Issuer che non richiede una WUA. Un Wallet Unit NON DEVE inviare una WUA a un Attestation Provider quando richiede un'attestazione non legata al dispositivo. Nota: Un Wallet Unit invia una WIA all'Attestation Provider indipendentemente dal fatto che le attestazioni emesse siano legate al dispositivo o meno.
- Un Wallet Provider DEVE garantire che la presentazione di una WUA sia crittograficamente vincolata al contesto specifico in cui è destinata a essere utilizzata. Nota: Come specificato in `OpenID4VCI`_, ciò si ottiene facendo in modo che la WUA firmata contenga essa stessa un nonce fornito dal PID Provider o dall'Attestation Provider durante il processo di emissione. In alternativa, il Wallet Unit presenta la WUA insieme a una Proof-of-Possession, costituita da una firma su tale nonce, creata con la chiave privata corrispondente a una delle chiavi pubbliche attestate nella WUA.
- Durante l'emissione di un PID o di un'attestazione legata al dispositivo (device-bound), il PID Provider o l'Attestation Provider DEVE verificare la WUA in conformità ai requisiti definiti nell'Appendice F.4 di `OpenID4VCI`_.
- Durante l'emissione di un PID o di un'attestazione legata al dispositivo (device-bound), il PID Provider o l'Attestation Provider DEVE ricevere una prova che il Wallet Unit possiede le chiavi private corrispondenti a tutte le chiavi pubbliche presenti nella WUA.
...
- Un Wallet Provider DEVE considerare tutti i fattori rilevanti, inclusi l'uso offline, l'interoperabilità e il rischio che una WUA diventi un vettore per tracciare l'utente, quando decide il periodo di validità di una WUA.
